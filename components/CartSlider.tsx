      ...
      <Transition show={open} as={Fragment}>
        <Dialog as="div" className="relative z-10" onClose={setCartSliderIsOpen}>
          ...
          <div className="fixed inset-0 overflow-hidden">
            <div className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
              <DialogPanel className="pointer-events-auto w-screen max-w-md">
                <div className="flex h-full flex-col items-center justify-center bg-white shadow-xl">
                  <Blinker />
                  <p>Loading cart...</p>
                </div>
              </DialogPanel>
                ...
                <DialogPanel className="pointer-events-auto w-screen max-w-md">
                  <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                                ...
                                <div className="ml-4 flex flex-1 flex-col">
                                  <div>
                                    <div className="flex justify-between text-base font-medium text-gray-900">
                                      ...
                                      <p className="ml-4">{formatCurrency({ amount: item.price_total })}</p>
                                    </div>
                                    <p className="mt-1 text-sm text-gray-500">{item.product.name}</p>
                                  </div>
                                  <div className="flex flex-1 items-end justify-between text-sm">
                    ...
                    <div className="border-t border-gray-200 py-6 px-4 sm:px-6">
                      ...
                      <div className="mt-6">
                        <Link href={cart.checkout_url || '/checkout'}>
                          <button
                            disabled={cartIsLoading || !cart.items.length}
                            className="flex h-12 w-full items-center justify-center rounded-md border border-transparent bg-cyan-600 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-cyan-700 disabled:cursor-not-allowed disabled:opacity-75"
                          >
                            {cartIsLoading ? <Blinker /> : 'Checkout'}
                          </button>
                        </Link>
                        {!cart.checkout_url && cart.items.length > 0 && (
                          <p className="text-sm text-gray-500 mt-2">Using default checkout path</p>
                        )}
                        {!cart.items.length && (
                          <p className="text-sm text-gray-500 mt-2">Add items to enable checkout</p>
                        )}
                      </div>
